...
export function ProjectTrendChart({ runs }: ProjectTrendChartProps) {
  const data = useMemo(() => {
    const now = Date.now();
    const days = 14;
  ...
  if (data.length < 2) {
    return (
      <div className="bg-card border border-card-border rounded-xl p-6">
        <h3 className="text-lg font-semibold mb-2">Pass Rate Trend</h3>
        <p className="text-sm text-muted">Not enough data points to display a trend. At least 2 days of runs are needed.</p>
      </div>
    );
  }
